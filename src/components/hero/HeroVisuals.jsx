...
const Card = ({ icon: Icon, title, delay = 0, className = "" }) => (
  <m.div 
    ...
    className={`absolute bg-white/90 border border-white/70 shadow-[0_8px_24px_rgb(0,0,0,0.08)] rounded-2xl p-3 sm:p-4 flex items-center gap-3 z-30 transform-gpu will-change-transform transition-transform duration-300 hover:scale-105 ${className}`}
  >
...
export default function HeroVisuals() {
  return (
    <div className="relative w-full h-[500px] sm:h-[600px] lg:h-[700px] flex items-center justify-center mt-10 lg:mt-0">
      ...
      {/* Floating Elements */}
      <div className="absolute inset-0 pointer-events-none z-30 transform-gpu will-change-transform">
        ...
        >
          <div className="w-full bg-blue-300 rounded-t-lg shadow-sm" style={{ height: '40%' }}></div>
          <div className="w-full bg-blue-600 rounded-t-lg shadow-sm" style={{ height: '70%' }}></div>
          <div className="w-full bg-emerald-600 rounded-t-lg shadow-sm relative" style={{ height: '100%' }}>
            {/* Arrow */}
